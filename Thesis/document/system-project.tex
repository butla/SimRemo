...
\subsubsection{Porównanie działania Jackson i~JSON.NET(TODO)}
W~kolejnych fragmentacj znajduje się zestawienie działania
Zestawienie ich działania dla kilku przypadków zestawione jest we fragmentach \ref{kod:serilization-objects}, \ref{kod:serialization-arrays}, \ref{kod:serialization-lists}, \ref{kod:serialization-dicts}
...
Różnice w~serializacji obu bibliotek: 
\begin{itemize}
	\item różni się nazwa specjalnego pola
	\item oznaczenia w~JSON.NET zawierają też nazwę biblioteki DLL
	\item styl oznaczania tablic dość się różni
	\item oznaczanie typów generycznych, Java tego nie wspiera, to mogłoby być probleme tłumacząc z Javy do C\#, w drugą stronie nie
\end{itemize}

Żeby wszystko działało trzeba by zrobić to to i to.

Nawet nie zagłębiałem się w głębsze funkcje, obie biblioteki mają ich dużo, ale nie są one bardzo potrzebne przy normalnych danych, jakie przekazujemy. Takie funkcje, to np. zapisywanie referencji do obiektów znajdujących się w pamięci -- coś, co nie ma prawa działać przy przenoszeniu danych na inna platformę.

A nie zagłębiałem się, bo o tablice się potknąłem, które są dość podstawowym zapotrzebowaniem.

\subsubsection{Co zrobiłem (TODO)}
Ostatecznie rzeczy które zrobiłem:
...
I działa to tak, jak pokazywałem wcześniej.
