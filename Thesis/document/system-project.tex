...
Serwer i~klient porozumiewają się przy użyciu JSON-RPC -- standardu zdalnego wywoływania metod opartego o~JSON, opisanego w~punkcie~\ref{json-rpc}.
W~ogólności system pozwala na wystawienie publicznych metod zwykłego javowego obiektu ,,na zewnątrz'' systemu Androida.
Taki wystawiany obiekt (zwany od teraz obiektem zdalnym) jest powiązany z~dowolnym portem TCP swojego.
Program na Androidzie wystawiający obiekty zdalne działa teraz jako serwer.
 
Dla przykładu można wziąć to i to i wystawić tak i tak.
Ogólnie można wziąć dowolny interfejs na Androidzie, jakąś jego implementację i wystawić na zewnątrz. Po stronier c\# trzeba mieć interfejs i zgodne klasy. I wszystko działa. Jeśli ktoś by chcciał koprzystać z klienta jsonrpc4j albo innego klienta JSON-RPC serializującego Jacksoinem to nie ma problemu. Jeśli nie potrzebuje oznaczania typów, to może używać dowolnego klienta JSON-RPC tak długo, jak ma odpowiednią interpretację. Bo nie musi być bezpośredniego mapowania typów, ale wiadomości JSON muszą móc być interpretowane dobrze (może takie rzeczy w punkcie o tłumaczeniu typów?).
...
Przygotowana przeze mnie część Androidowa to jedna klasa -- JsonRpcBasicServer -- dodana do jsonrpc4j. Więcej o~niej w~punkcie XXXX W~przypadku Androida jest to jedna zmodyfikowana klasa jsonrpc4j (JsonRpcBasicServer). 
\begin{figure}
	\centering
...
A~wspólne działanie klas na diagramie~\ref{fig:sequence-diagram}.
\begin{figure}
	\centering
		\includegraphics[angle=90,origin=c,scale=0.8]{img/schematy/diagram-sekwencji.pdf}
	\caption{Diagram sekwencji systemu.}
