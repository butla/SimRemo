...
TODO
Tak naprawdę, to opiszę, co musi zostać zrobione, aby zrealizować cele z punktu BLABLA. Powstanie też projekt rozwiązania. Poszczególne jego elementy będą przeanalizowane. Powiem, jak można je zrobić, jakie są przeszkody i ograniczenia, a także dla części dostarczę prototypy.

Czyli najpierw wymagania i projekt, potem opisy/realizacja części wraz z opisem, jaką część wymagań one spełniają.
...
Może też zrobić jakiś parser nakładkę? Przetwarzał by on gotowy dokument JSON przygotowany przez JSON.NET na format Jacksona i na odwrót.

A może jednak nie ma takiej tragedii i da radę ogarnąć to tym JsonConverterem? Ale notki o skomplikowaniu serializacji powinny pozostać, bo wszystkich przypadków wspierać nie dam rady.
