...
Będę się kierował praktycznością i leniwstwem. System może nie będzie idealny, ale będzie bardzo wygodny w użyciu.

Trudno mi samemu nie zauważyć, że praca zawodowa wywarła wpływ na mój proces twórczy (w odniesieniu do niniejszej pracy). Techniki, które poznałem oraz środowisko, w~którym działałem nastawiło mnie na efekty (Nastawienie na wydajność, niezawodność, powtarzalność)Trochę się nauczyłem (TDD, Spike'owanie i inne techniki) i mam nadzieję, że będę się kierował praktycznością, dobrymi praktykami, metodykami i lenistwem (cytat z Billa Gatesa).

System, który ostatecznie stworzę najprawdopodobniej nie będzie się jeszcze nadawałraczej nie będzie idealny. Nie interesuje mnie jeszcze wprowadzenie do produkcji, zapewnianie bezpieczeństwa itp. Może też są już inne rozwiązania, które pozwalają na to samo. Ale moje ma być wygodne naj
może powstanie coś, czego jeszcze na świecie nie ma (albo nie jest tak przyjemne w użyciu). Co prawda pewnie nie będzie uniwersalne, ale będzie lepszym niż dostępne rozwiązaniem pewnej klasy problemów (jak już ktoś ma te kawałki).
...
\section{Cele pracy}
W tym miejscu dokładnie nakreślę cele tej pracy. Z jednej strony mówi to Tobie, czytelniku, co zostanie tutaj dokonane, z drugiej - wytycza mi ścieżkę i będzie służyło za przypomnienie w trakcie badań, projektowania, implementacji i testów mojego produktu.
