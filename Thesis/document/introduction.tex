...
Nie jest to trywialny proces i ma swoje problemy.
...
Do tekstu (czytelne dla człowieka, np. XML) albo binarne (nieczytelne).
...
Mogą być bardziej uniwersalne binarne formaty. Mogą bić problemy little/big endian.
Można jedną rzecz zapisać do XMLa na wiele różnych sposobów.

Problemy, które trzeba rozwiązywać: (i tu wymienione cykle itp)
Wiąże się z problemami cyklów i dużych sieci. Jak mamy obiekty powiązane, to jak poznamy, że są powiązane, a nie że są podobiektami większego i ten większy może się bez nich obyć?

Rozwiązania tych problemów: (i tu jakieś techniki, ale to chyba się wszystko wiąże z bindingiem)
Ręczne dopasowywanie serializacji. Customowe rozwiązania oraz własne dopasowywanie bindingu klas.
Oznaczanie pól jako transient itp
...
Obiekty danych mogą być przechowywane różnie w róznych językach, nie muszą być jednoznacznie przetłumaczalne (niezgodność kodów bajtowych platform wirtualnych). Przez tłumaczenie do formy pośredniej a potem z tej pośredniej na drugi język można często coś przetłumaczyć.
