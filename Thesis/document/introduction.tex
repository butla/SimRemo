...
\subsection{Android(TODO)\cite{learningandroid}}
...
Android jest kompletną, otwartą (open-source) platformą dla urządzeń mobilnych opartą o~Linux. Został stworzony przez organizację Open Handset Alliance. Organizacja skupia wielu operatorów komórkowych, producentów elektroniki, telefonów i~oprogramowania oraz firmy komercjalizacyjne \cite{oha}. Pomysłodawcą uformowania oraz liderem tej organizacji jest firma Google. Jej misją jest przyspieszenie innowacji na platformach mobilnych oraz dostarczenie konsumentom pełniejszego, tańszego i~ogólnie lepszego doznania z~korzystania z~telefonu. Wszystko to za sprawą Androida, który dodatkowo jest całkowicie otwarty.
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=1.00\textwidth]{img/android_architecture.jpg}
...
(TODO)
Trzeba powiedzieć, że pisanie kodu natywnego nie jest normalnym, czy też może zalecanym, podejściem do tworzenia aplikacji na Androidzie. 
Można na nim pisać wszystko tak, jak na Linuxie, ale normalnie jest to trochę ograniczone i trzeba rootować (źródło).
Zalecana jest java, tu o Javie i Dalviku. Chociaż teraz można już natywne aplikacje całe (NDK albo NativeActivity). I nie jest to tak niewydajne jak kiedyś.

Dalvik, Android runtime (obrazek dla dalvika)

W teorii można by na Dalviku odpalać wszystko, co się kompiluje do kodu bajtowego Javy.

Można też inaczej, np. SL4A i interpretery Pythona, Rubiego itp. Bo one pewnie są komilowne do kodu bajtowego Javy
...
Ładnie udokumentowane wszystko
