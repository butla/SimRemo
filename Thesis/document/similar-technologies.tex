	...
	\centering
		\begin{tabular}{ | c | c | p{8cm} |}
			\hline
				...
				\hline
				Klasyfikacja & Ogólna & Czym dana technologia jest, co potrafi (z mojej dziedziny zainteresowania)? Czy tłumaczy kod, czy serializuje, czy może być klientem RPC, czy może być serwerem RPC?\\
				\hline
		\end{tabular}
...
Większość testów wykonywana jest na systemie Windows 7 Professional x64.
Te, które tego wymagają odbywają się na emulowanym (w~Windowsie) Androidzie 4.4.2, który szerzej jest opisany w~punkcie~\ref{android-emulator}.

Wśród wszystkich bibliotek i~narzędzi, będę szukał takich, które mogę wkomponować w~moje rozwiązanie.

Testy polimorfizmu danych:
Tylko zserializowanie i odtworzenie hardcorowej talblicy.

Będą dwa testy, różniące się tylko wejściowymi danymi. ,,Dane'' to tak naprawdę jeden obiekt.
Kroki testu:
\begin{enumerate}
...
Te obiekty:
\begin{lstlisting}[frame=single, caption={Przykładowa klasa danych używana w testach polimorfizmu zdalnych metod}, label=kod:testDataA]
public class DaneA
{
	public int liczbaA = 13;
	public string tekst = "cokolwiek";
}
\end{lstlisting}

\begin{lstlisting}[frame=single, caption={Przykładowa klasa danych używana w testach polimorfizmu zdalnych metod}, label=kod:testDataB]
...
Testy polimorfizmu metod:
Przekazanie obiektu dziedziczącego w miejsce konkretnego argumentu.
Zwrócenie tablicy.
Metoda przeciążona.

OPISAĆ TESTY POLIMORFIZMU (KLASY, STRUKTURY, METODY)
...
%Chyba to za duże, wystarczy dziedziczący obiekt i generyczna tablica (int, string, obiekt A, obiekt B, tablica)
