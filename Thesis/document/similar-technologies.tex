	...
	\centering
		\begin{tabular}{ | c | c | p{8cm} |}
			\hline
				...
				\hline \hline
				Klasyfikacja & Ogólna & Czym dana technologia jest, co potrafi (z mojej dziedziny zainteresowania)? Czy tłumaczy kod, czy serializuje, czy może być klientem RPC, czy może być serwerem RPC? Poza tym, do których dziedzin z~tej tabeli się zalicza?\\
				\hline
...
Średnia.
Wymaga odnalezienia się w~dużej ilości przestarzałych stron, zawierających sprzeczne informacje.
...
(TODO)
Biblioteka umożliwiająca działanie jako serwer lub klient JSON-RPC 2.0 napisana w~Javie.
...
\subsection{Testy (TODO -- odpal jeszcze raz)}
...
%Dodać ogólnie open source do klasyfikacji
...
\subsection{Ocena}
%Może przerobić to na tabelę? Cecha / Ocena / Komentarz
\begin{description}
\itemtitle{Klasyfikacja}
Klient RPC, Serwer PRC, Serializacja, Open Source.
\itemtitle{Docelowa platforma}
Poza obszarem zainteresowania -- Java SE. Android teoretycznie powinien być możliwy, ale nie jest.
\itemtitle{Łatwość instalacji}
Może być... chyba muszę jeszcze przejrzeć notki.
\itemtitle{Łatwość użycia}
Sensowne i łatwe, ale może nie oczywiste.
%Może zamiast standardowość i współpraca zrobić pole zgodność?
\itemtitle{Standardowość}
...
Dlatego zgodność z~drugą stroną możliwa jest tylko w~przypadku, gdy tak samo koduje ona informacje o~typach. Jest to możliwe chociażby wtedy, kiedy używałaby tak samo skonfigurowanego Jacksona do serializacji.
\itemtitle{Współpraca}
To samo, co w ,,Standardowości''.
\itemtitle{Code reuse}
Jest -- można podłączać zewnętrzne biblioteki i używać klas z nich w naszych metodach. Jeśli tylko są jakoś serializowalne.
\itemtitle{Modularność (taka rozszerzalność)}
Jest -- mogę dodać nowe klasy dziedziczące po argumentach zdalnej metody i~metoda może je obsługiwać, jeśli tylko podczepimy te klasy po obu stronach.
\itemtitle{Integracja z Androidem}
Jest.
\itemtitle{Polimorfizm metod}
TODO -- sprawdź przeładowania.
\itemtitle{Polimorfizm danych}
Jest.
\end{description}
...
Niestety, mimo obietnicy twórcy, nie można na Androidzie stworzyć serwera, ale zdaje się, że można by to naprawić przez wycięcie fragmentów kodu (można to zrobić, bo kod jest otwarty), które i~tak nie są potrzebne w~moim scenariuszu użycia.
